/**
 * bulk-seed.ts에서 JSON 파싱 실패한 토픽을 재시도.
 * Gemini JSON mode(responseMimeType: 'application/json')로 강제해 신뢰성 높임.
 *
 * 사용법:
 *   GEMINI_MODEL=gemini-2.5-flash corepack pnpm exec tsx scripts/retry-failures.ts \
 *     "토픽1" "토픽2" "토픽3"
 *
 * 또는 슬러그 비교로 누락된 토픽만 자동 검출:
 *   corepack pnpm exec tsx scripts/retry-failures.ts --auto
 */
...
// bulk-seed.ts와 같은 토픽 리스트 (slug 비교용)
const ALL_TOPICS: Array<{ category: ContentCategory; topic: string; keywords: string[] }> = [
...
async function generatePostJsonMode(topic: string, category: ContentCategory, keywords: string[]) {
  ...
  console.log(`  [KO json-mode] ${((Date.now() - t0) / 1000).toFixed(1)}s, ${text.length} chars`)
  const parsed = extractFirstJsonObject<any>(text)
  if (!parsed.title || !parsed.content) throw new Error('missing title/content')
  return parsed
}

async function translateToEnglishJsonMode(koPost: any) {
  const prompt = `Translate the following Korean blog post to natural, fluent English. Keep the markdown formatting intact.

KOREAN TITLE: ${koPost.title}
KOREAN EXCERPT: ${koPost.excerpt}
KOREAN CONTENT (markdown):
${koPost.content}

Output JSON: {"title": "...", "excerpt": "...", "content": "...markdown body...", "seoTitle": "...", "seoDescription": "..."}`
  const t0 = Date.now()
  const result = await jsonModel.generateContent(prompt)
  const text = result.response.text()
  console.log(`  [EN json-mode] ${((Date.now() - t0) / 1000).toFixed(1)}s, ${text.length} chars`)
  const parsed = extractFirstJsonObject<any>(text)
  if (!parsed.title || !parsed.content) throw new Error('missing translation title/content')
  return parsed
}

async function findMissingTopics() {
  // Turso에 있는 글 제목들과 토픽 비교 — 가장 단순한 매칭은 첫 단어
  ...
  const missing = ALL_TOPICS.filter(t => {
    // 토픽의 첫 키워드가 어떤 기존 글 제목에 포함되는지 확인
...
async function main() {
  ...
  let toRetry: typeof ALL_TOPICS

  if (args[0] === '--auto') {
    toRetry = await findMissingTopics()
    console.log(`Auto-detected ${toRetry.length} missing topics`)
  } else if (args.length > 0) {
    // 명시한 토픽 substring으로 매칭
    toRetry = ALL_TOPICS.filter(t => args.some(arg => t.topic.includes(arg)))
    console.log(`Matched ${toRetry.length} topics from args`)
  } else {
    console.log('Usage: tsx scripts/retry-failures.ts [--auto | "토픽 substring" ...]')
    process.exit(1)
  }
  ...
  for (let i = 0; i < toRetry.length; i++) {
    const { category, topic, keywords } = toRetry[i]
    console.log(`\n[${i + 1}/${toRetry.length}] [${category}] ${topic.slice(0, 60)}...`)

    try {
      const ko = await generatePostJsonMode(topic, category, keywords)

      const slug = (ko.slug || `${category}-retry-${Date.now()}`).slice(0, 60)
      const exists = await turso.execute({ sql: 'SELECT id FROM Post WHERE slug = ? LIMIT 1', args: [slug] })
      const finalSlug = exists.rows[0] ? `${slug}-${Date.now()}` : slug

      const postId = randomCuid()
      const idxInOriginal = ALL_TOPICS.findIndex(t => t.topic === topic)
      const cover = COVERS[category][idxInOriginal % 10]
      const tagsArr = [category, ...(ko.tags || []).filter((t: string) => t && t !== category)]
      const publishedAt = sqliteDate(new Date(Date.now() - i * 60 * 60 * 1000))
      ...
      await turso.execute({
        ...
        args: [postId, ko.title, finalSlug, ko.content, ko.excerpt || '', tagsArr.join(','),
               ko.seoTitle || ko.title, ko.seoDescription || ko.excerpt || '', cover,
               'n잡러 프리랜서', publishedAt],
      })
      console.log(`  ✓ Saved: ${finalSlug}`)

      try {
        const en = await translateToEnglishJsonMode(ko)
        const trId = randomCuid()
        await turso.execute({
          ...
          args: [trId, postId, en.title, en.content, en.excerpt || '', en.seoTitle || en.title, en.seoDescription || en.excerpt || '', cover],
        })
    ...
    if (i < toRetry.length - 1) await new Promise(r => setTimeout(r, 1500))
  }

...
export default function AffiliateProductsPage() {
  ...
  const [autoGenerateDraft, setAutoGenerateDraft] = useState(false)
  const [generatingProductId, setGeneratingProductId] = useState<string | null>(null)

  const generateDraft = async (productId: string): Promise<{ slug: string; blog: string } | null> => {
    setGeneratingProductId(productId)
    try {
      const res = await fetch(`/api/admin/affiliate-products/${productId}/generate-draft`, {
        method: 'POST',
      })
      if (!res.ok) {
        const err = await res.json().catch(() => ({}))
        alert(`초안 생성 실패: ${err.error ?? res.statusText}`)
        return null
      }
      const data = await res.json()
      return { slug: data.post?.slug, blog: data.blogCategory }
    } catch (e) {
      alert('초안 생성 중 네트워크 오류')
      return null
    } finally {
  ...
  const handleGenerateDraftClick = async (productId: string, productName: string) => {
    if (!confirm(`"${productName}"에 대한 블로그 초안을 자동 생성합니다.\n\n· Claude API 비용 약 $0.10-0.20\n· DRAFT 상태로 저장 (자동 발행 안 됨)\n· /admin posts 테이블에서 검토 후 발행하세요\n\n진행할까요?`)) {
      return
    }
    const result = await generateDraft(productId)
    if (result) {
      const blogLabel = result.blog === 'dev' ? '개발막차' : 'n잡러'
      alert(`초안이 ${blogLabel} 블로그용으로 생성되었습니다.\n\n/admin 페이지의 posts 테이블에서 'DRAFT' 상태로 확인 가능합니다.`)
    }
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const newProductId = saved.data?.product?.id ?? saved.data?.id

      // 신규 등록이고 자동 생성 옵션이 켜져 있으면 곧바로 초안 생성
      if (!editingProduct && autoGenerateDraft && newProductId) {
        alert(`등록 완료. 이제 Claude로 초안을 생성합니다 (1-2분 소요).`)
        const result = await generateDraft(newProductId)
        if (result) {
          const blogLabel = result.blog === 'dev' ? '개발막차' : 'n잡러'
          alert(`초안 생성 완료!\n${blogLabel} 블로그용으로 작성됨.\n\n/admin posts 테이블에서 DRAFT 확인 후 본인 경험 추가하고 발행하세요.`)
        }
      } else {
            ...
            {products.map((product) => (
              <tr key={product.id}>
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                  <button
                    onClick={() => handleGenerateDraftClick(product.id, product.name)}
                    ...
                    title="Claude로 블로그 초안 자동 생성"
                  >
                    {generatingProductId === product.id ? '생성 중...' : '초안 생성'}
                  </button>
                  <button
        ...
        <div className="fixed inset-0 bg-gray-500 bg-opacity-75 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg max-w-2xl w-full p-6 max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {!editingProduct && (
                <label className="flex items-start gap-2 p-3 rounded-md border border-purple-200 bg-purple-50 cursor-pointer hover:bg-purple-100 transition-colors">
                  <input
                    type="checkbox"
                    checked={autoGenerateDraft}
                    onChange={(e) => setAutoGenerateDraft(e.target.checked)}
                    className="mt-0.5 h-4 w-4 rounded text-purple-600 focus:ring-purple-500"
                  />
                  <div className="text-sm flex-1">
                    <div className="font-medium text-purple-900">등록 후 자동으로 초안 글 생성</div>
                    <div className="text-xs text-purple-700 mt-0.5">
                      Claude가 이 상품의 dev/sidehustle 블로그 적합성을 판단하고 도구 소개 톤의 초안 글을
                      DRAFT 상태로 작성합니다. (비용 약 $0.10-0.20, 1-2분 소요)
                    </div>
                  </div>
                </label>
              )}
